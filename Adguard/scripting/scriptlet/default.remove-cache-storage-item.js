...
const uBOL_removeCacheStorageItem = function() {

const scriptletGlobals = {}; // jshint ignore: line

const argsList = [["yt-appshell-assets","/\\/s\\/desktop\\/[0-9a-f]{8}\\/(?:cs|j)sbin\\//"],["yt-appshell-assets","/\\/s\\/player\\/[0-9a-f]{8}\\/(?:player_ias\\.vflset\\/|www-player\\.css)/"]];
...
for ( const i of todoIndices ) {
    try { removeCacheStorageItem(...argsList[i]); }
    catch(ex) {}
}
argsList.length = 0;
...
if ( typeof wrappedJSObject !== 'object' || targetWorld === 'ISOLATED' ) {
    return uBOL_removeCacheStorageItem();
}
...
// Firefox
{
    ...
    let script, url;
    try {
        page.uBOL_removeCacheStorageItem = cloneInto([
            [ '(', uBOL_removeCacheStorageItem.toString(), ')();' ],
            { type: 'text/javascript; charset=utf-8' },
        ], self);
        const blob = new page.Blob(...page.uBOL_removeCacheStorageItem);
    ...
    delete page.uBOL_removeCacheStorageItem;
}
